...
with tab2:
    ...
    with kc1:
        sel_ks = st.multiselect(
            "비교할 종목 선택 (최대 10개)",
            options=list(KOREAN_STOCKS.keys()),
            default=["삼성전자", "SK하이닉스", "현대차", "카카오", "네이버"],
            max_selections=10,
            key="t2_ks"
        )
    with kc2:
        period_t2 = st.selectbox(
            "📅 기간", list(PERIOD_OPTIONS.keys()), index=3, key="t2_period"
        )

    if not sel_ks:
        st.warning("⚠️ 종목을 1개 이상 선택하세요.")
    else:
        ks_tickers  = [KOREAN_STOCKS[n] for n in sel_ks]
        ks_name_map = {KOREAN_STOCKS[n]: n for n in sel_ks}

        with st.spinner("📡 한국 주식 데이터 불러오는 중..."):
        ...
        if err_ks or ks_close is None:
            ...
            st.plotly_chart(
                draw_return_bar(ks_ret, "🏆 기간 수익률 랭킹"),
...
with tab3:
    ...
    with uc1:
        sel_us = st.multiselect(
            ...
            default=["애플 (AAPL)", "엔비디아 (NVDA)", "테슬라 (TSLA)",
    ...
    with uc2:
        period_t3 = st.selectbox(
            "📅 기간", list(PERIOD_OPTIONS.keys()), index=3, key="t3_period"
        )

    if not sel_us:
        st.warning("⚠️ 종목을 1개 이상 선택하세요.")
    else:
        us_tickers  = [US_STOCKS[n] for n in sel_us]
        us_name_map = {US_STOCKS[n]: n for n in sel_us}
        ...
        if err_us or us_close is None:
            ...
            colors_us = px.colors.qualitative.Plotly
            ...
            st.plotly_chart(
                draw_return_bar(us_ret, "🏆 기간 수익률 랭킹", margin_l=220),
                use_container_width=True
            )
...
with tab4:
    ...
    with ic1:
        sel_idx = st.multiselect(
            ...
            default=list(INDEX_STOCKS.keys()),
            key="t4_idx"
        )
    with ic2:
        period_t4 = st.selectbox(
            "📅 기간", list(PERIOD_OPTIONS.keys()), index=3, key="t4_period"
        )

    if not sel_idx:
        st.warning("⚠️ 지수를 1개 이상 선택하세요.")
    else:
        idx_tickers  = [INDEX_STOCKS[n] for n in sel_idx]
        idx_name_map = {INDEX_STOCKS[n]: n for n in sel_idx}
        ...
        if err_idx or idx_close is None:
            ...
            with col_right:
                ...
                if ret_corr.shape[1] >= 2:
                    ...
                    st.info("ℹ️ 상관관계는 2개 이상의 지수를 선택해야 표시됩니다.")
